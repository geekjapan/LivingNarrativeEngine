...
## Purpose
TBD - created by archiving change add-intervention. Update Purpose after archive.
## Requirements
### Requirement: Intervention スキーマ
Intervention は企画書 §14.8 に準拠し、`id`(project 内一意、`int_NNNN` ゼロパディング通番、spec-foundation §3)、`turn`(発生ターン番号)、`user_role`(介入時点の `user_mode`。企画書 §8: `watcher` / `assistant_gm` / `full_gm` / `author` / `player_character` / `god`)、`type`、`target`、`content`(自由文または要約テキスト)、`constraints`(キー・バリューの追加制約)、`visibility`(spec-foundation §4.2 の enum)のフィールドを持たなければならない(SHALL)。`type` は企画書 §10.7 に列挙された15種(`scene_directive` `character_directive` `world_directive` `event_injection` `probability_bias` `tone_control` `pacing_control` `reveal_control` `hidden_truth_edit` `canon_edit` `dice_roll_request` `stop_condition` `scene_pivot` `relationship_edit` `memory_edit`)のいずれかでなければならない(SHALL)。`target` はネストされた Pydantic モデル(`InterventionTarget`)であり(SHALL)、`kind`(spec-foundation §5.1 の StateDiff target enum(`world` / `character` / `scene` / `reader_state` / `canon` / `gm_vault` / `relationship`)に dice_roll_request 用の `roll` を加えた enum)と、任意の `id`(対象を一意特定する id 文字列。`world` のように対象が単一でありインスタンス id を要しない場合は省略可、`relationship` では spec-foundation §3 の複合キー `<from_id>__<to_id>` 形式)を持たなければならない(SHALL)。`Intervention` のトップレベルに `target` の enum 値と id を並列で持つ flat 構造としてはならない(MUST NOT)。Pydantic v2 モデルとしてロード時に検証されなければならない(SHALL、spec-foundation §5)。
...
Intervention システムは、LLM Interpreter を経由せず、呼び出し元(CLI フラグやテストコード)が型付き Intervention フィールドを直接指定して Intervention を構築できる経路を提供しなければならない(SHALL)。呼び出し元が直接指定できるのは `type`/`target`/`content`/`constraints`/`visibility` のみであり、`id`/`turn`/`user_role` は Interpreter 経路と同一の規則で、常にそのターンの実行コンテキスト(現在のターン番号・現在の `user_mode`・id 採番機構)からシステムが値を割り当てなければならない(SHALL)。呼び出し元が `id`/`turn`/`user_role` の値を供給してきた場合、その値を採用してはならず(MUST NOT、`user_role` の偽装・誤った turn の割り当て・`int_NNNN` の重複を防ぐため)、常に無視・破棄した上で実行コンテキスト由来の値で補完しなければならない(SHALL)。この経路は Interpreter と同一のスキーマ検証・permission hook を適用しなければならない(SHALL)。
...
#### Scenario: 呼び出し元が供給した user_role は採用されない
- **WHEN** 現在の `user_mode` が `author` のセッションで、呼び出し元が `user_role: god` を含む入力で直接入力パスの Intervention 構築 API を呼ぶ
- **THEN** 供給された `user_role: god` は無視・破棄され、生成される Intervention の `user_role` には実行コンテキスト由来の `author` が割り当てられる。permission 判定・履歴記録が偽装された role で行われることはない
...
確定した Intervention は type に応じて次のように実行主体へルーティングされなければならない(SHALL): `scene_directive` は現在シーンのコンテキスト(当該シーンの `active_characters` に属する全 Character Agent および World Simulator)へシーン演出の制約として渡される。`character_directive` は対象キャラクターのコンテキストのみに渡され、他キャラクターや Narrator のコンテキストには含まれてはならない(spec-foundation §4.3 準拠)。`world_directive` および `event_injection` は World Simulator へ渡される。`tone_control` および `reveal_control` は Narrator 制約として渡される。`dice_roll_request` は Conflict Resolver 経由で Random Engine の判定要求として渡される。`canon_edit` および `hidden_truth_edit` は State Manager 経由で state diff エントリとして生成され、`visibility`(`canon` は canon 相当、`hidden_truth_edit` は `gm_only` 既定)が尊重されなければならない(SHALL、D107: state 変更は全て diff 経由)。この際 State Manager は当該 intervention を `cause` に持つ合成 resolved event を生成し、その event id を state diff エントリの `source_event` に用いなければならず(SHALL)、intervention id を `source_event` に直接格納してはならない(MUST NOT、provenance は常に event 経由とし State Manager の「source_event なし拒否」規則と整合させる)。`reveal_control` の Narrator 制約としての伝達は、Requirement「reveal_control の意味論」に定める BuildDiff スロット(spec-foundation §6 フェーズ8内、agent-runtime の State Manager 実装。D113)での Reader State 昇格制御を代替しない(SHALL NOT)。両者は独立した効果であり、いずれも実装されなければならない(SHALL)。
...
- **THEN** 当該 intervention を `cause` に持つ合成 resolved event が生成され、その event id を `source_event` として参照する state diff エントリ(target: `canon`)が生成され、Canon への直接書き換えは発生しない
...
プロジェクト全体で累積する Intervention 履歴インデックス(`interventions.yaml`)を、ターンをまたいで維持しなければならない(SHALL)。各エントリは intervention id・発生ターン・type・そのターンで生じた結果(event id / state diff id 等)への source reference・`superseded_by_rerun`(既定 `false`)フラグを持ち、任意の intervention について「何が実際に起きたか」を追跡可能にしなければならない(SHALL)。あるターンの intervention 群に対応するエントリは、そのターンの Commit フェーズ完了時点(spec-foundation §6 フェーズ8、event id と候補 state diff id の双方が確定した後)で、source reference を含めて一度だけ書き込まれなければならない(SHALL)。Intervene フェーズ(フェーズ2、`intervention.yaml` 保存時点)では `interventions.yaml` への書き込みを行ってはならない(SHALL NOT)。`rerun_turn`(`add-session-autonomy`、D112: 旧 artifact を `turn_NNNN_discarded_<n>` へ退避)によってあるターンが再実行された場合、破棄された attempt の event id / state diff id を参照している既存エントリは、`session-autonomy` の rerun 操作により `superseded_by_rerun: true` に更新される以外、内容を変更されてはならない(SHALL、上書き禁止の原則を維持する)。再実行後の Commit 完了時点で、新しい event id / state diff id を参照する新規エントリが追記されなければならない(SHALL)。ターンが `pending_review`/`stopped_for_review` のまま Commit フェーズを終えた場合、当該ターンのエントリ書き込みはターン解決まで遅延しなければならない(SHALL)。当該ターンが session-autonomy のレビュー決定(`accept_all`/`reject_all`/`partial`/`edit`)により解決された時点で、決定を処理する session-autonomy 側が、確定した event id と実際に適用された state diff 内容に基づく source reference を持つエントリを一度だけ追記しなければならない(SHALL、遅延書き込みの確定点)。`reject_all` で解決された場合もエントリは追記され、source reference は適用された変更が空であることを示す(SHALL、監査可能性のため履歴から intervention が消えない)。`rerun_turn` は上記の supersession 規則に従う。
...
#### Scenario: レビューで解決されたターンでも履歴エントリが追記される
- **WHEN** intervention を含むターンが `pending_review` で停止し、その後 `accept_all`(または `partial`/`edit`/`reject_all`)で解決される
- **THEN** 解決時点で当該 intervention 群のエントリが `interventions.yaml` へ一度だけ追記され、source reference は確定した event id と実際に適用された diff 内容(`reject_all` では適用変更なし)を示す
