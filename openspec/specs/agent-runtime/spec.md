...
## Purpose
TBD - created by archiving change add-agent-runtime. Update Purpose after archive.
## Requirements
...
### Requirement: Conflict Resolver の roll severity 指定
Conflict Resolver は、衝突解決のため random-engine へ判定(roll)を要求する際、生死に関わる行動や取り返しのつかない結果を伴うなど争われている行動の重大性が高いと判断した場合、当該 roll 呼び出しに明示的な `severity: critical`(spec-foundation D123、Roll の任意フィールド)を付与してよい(MAY)。この severity 判定は Conflict Resolver 自身の裁量による呼び出し側判断であり(SHALL、spec-foundation D123)、random-engine は severity を自動算出しない。severity を明示しない場合、当該 roll の severity は random-engine 側の既定値(`normal`)のままとなる。

#### Scenario: 生死に関わる行動の roll に severity: critical が付与される
- **WHEN** Conflict Resolver が、キャラクターの生死に関わる行動(life-or-death action)を含む衝突を解決するために random-engine へ判定を要求する
- **THEN** Conflict Resolver は当該 roll 呼び出しに `severity: critical` を明示的に指定する
...
State Manager は turn-pipeline の BuildDiff スロットの実装として登録されなければならない(SHALL、D113)。State Manager は、resolved event 列 + 当該ターンの intervention 群(`intervention.yaml` に確定した Intervention のリスト。無介入時は空リスト)+ 全状態を入力として state diff 候補を state-model の StateDiff 形式で生成しなければならない(SHALL)。生成された各 diff 変更は、生成時点で現在状態に対して検証されなければならない(SHALL)。`source_event` を持たない state diff の変更は、生成時点で拒否されなければならない(SHALL)。intervention に直接由来する変更(`canon_edit`/`hidden_truth_edit`/`reveal_control: reveal-now` による昇格)については、State Manager は当該 intervention を `cause` に持つ合成 resolved event を生成し、その event id を `source_event` に用いなければならず(SHALL)、intervention id を `source_event` に直接格納してはならない(MUST NOT、provenance は常に event 経由。add-intervention の Type 別ルーティング要件と同一契約)。当該ターンで確定した `reveal_control`(must-not-reveal)により reader 可視への昇格が禁じられている事実を `target: reader_state` へ昇格させる変更候補は、state diff 候補に含めてはならない(SHALL NOT)。ある resolved event が対象キャラクターの死亡(fate resolved to death)を表す場合、State Manager が生成する対応する state diff 候補は当該キャラクターへの `target: character, path: status, op: set, value: dead` change を含まなければならない(SHALL、spec-foundation D123、CharacterState.status)。同様に、ある resolved event がシーンの終了(scene concludes)を表す場合、対応する state diff 候補は当該シーンへの `target: scene, path: status, op: set, value: ended` change を含まなければならない(SHALL、spec-foundation D123、SceneState.status)。State Manager の出力は、採用された state diff 候補に加え、拒否された変更候補とその理由を保持する `rejected_changes` リストを持たなければならない(SHALL)。
...
#### Scenario: キャラクター死亡の resolved event が status: dead の diff を生成する
- **WHEN** Conflict Resolver がキャラクターの死亡を表す resolved event を生成し、State Manager がこれを処理する
- **THEN** 生成される state diff 候補に、当該キャラクターの `target: character, path: status, op: set, value: dead` を持つ change が `source_event` 付きで含まれる

#### Scenario: シーン終了の resolved event が status: ended の diff を生成する
- **WHEN** Conflict Resolver がシーンの終了を表す resolved event を生成し、State Manager がこれを処理する
- **THEN** 生成される state diff 候補に、当該シーンの `target: scene, path: status, op: set, value: ended` を持つ change が `source_event` 付きで含まれる
