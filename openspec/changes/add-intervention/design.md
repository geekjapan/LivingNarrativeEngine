...
- 決定: Interpreter の response_schema は `interventions: list[Intervention]` + `confidence: float` + `interpretation_summary: str` とする。自由文のうち既知 type に明確に分類できなかった部分は、破棄せず `type: scene_directive` として `content` にそのまま残す。ただし LLM に生成させるのは各 `interventions[]` 要素の `type`/`target`/`content`/`constraints`/`visibility` のみであり、`id`(spec-foundation §3 の project 内一意通番)・`turn`・`user_role` は LLM 出力に含めない。これらはターン実行コンテキストからパイプライン統合層が確定値を補完したうえで、正規の `Intervention` モデルとして検証・確定する(直接入力パスも同一規則に従う。§構造化直接入力パス参照)。
