...
- ランタイム依存として `pydantic`(v2)・`pyyaml`・`typer` を導入する(spec-foundation §2 のスタックに準拠。`typer` は本 change の `init` コマンドが必要とする)。
...
- 任意の `llm_profiles`(名前付きプロファイル辞書)・`llm_bindings`(binding key → プロファイル名)フィールドをスキーマに追加する(spec-foundation D122)。`llm_bindings` のキーは既定の binding key 集合(`character:<char_id>` パターンを含む)に限定し、未定義プロファイル名への binding はロード時検証エラーとする。`init` が生成する既定 `project.yaml` にはこれらのフィールドを含めない(プロファイル未使用が既定)。
- workspace ディレクトリレイアウト(企画書 Appendix C 準拠)を定義し、`living-narrative init` コマンド(typer スケルトン)で最小の空ワールドテンプレートから新規プロジェクトを生成できるようにする。出力先が既存かつ空でないディレクトリの場合は上書きを拒否する(空ディレクトリは許容。`specs/project-workspace/spec.md` の Requirement「既存ディレクトリへの上書き拒否」と同一契約)。
