...
- [Risk] `review` の非対話フラグ体系が過度に複雑になり、実装・ドキュメントの負担が増える。
  → Mitigation: 第1バッチでは `accept`/`reject`/`rerun` を単純フラグ、`partial`/`edit` のみインデックス指定・パッチファイルという最小限の追加引数に留める(D2)。`partial` のインデックス選択は session-autonomy の GM レビューゲート契約(change のインデックス集合)にそのまま従う。
