...
- Simulate / Act / Resolve / BuildDiff / Check の 5 フェーズを Protocol による差し替え可能スロットとして定義する(D108/D113: レジストリ辞書、plugin loader は作らない)。
- 各スロットの最小組み込み実装を追加し、mock provider だけでターンが最初から最後まで実行できるようにする:
  ...
  - BuildDiff: resolved events・そのターンの intervention・全状態から state diff 候補を生成する最小実装(旧来 Commit 内部で行っていた diff 生成ロジックをここへ移す。D113。agent-runtime の State Manager が本実装へ差し替わる)。
...
- Commit フェーズを追加する: BuildDiff スロットが生成した state diff 候補を受け取り、state-model の diff 適用に渡す固定ロジック(diff 自体は生成しない。D113)。
  適用するか pending にするかは、この change ではターン実行 API のランタイムパラメータである単純な commit-mode(`auto` / `review`)で決める(session-autonomy の判断ロジックは対象外。D118: project.yaml のフィールドにはしない)。
...
- `meta.yaml` に `status`・フェーズ別所要時間・LLM 呼び出し回数・`llm_tokens_total`(取得可能分の全呼び出し合計、取得不能時省略)・呼び出しごとの記録(binding key・プロファイル名・model 名。D122: 1ターン内で複数プロファイルが使われうるため単一の model 名ではなく呼び出しごとに記録する)・prompt hash・`rng_draws_consumed`(そのターンで消費した draw 数)・pipeline version を記録し、meta.yaml は完了マーカーとして最後に書く(D111)。
- ターンステータスモデル(`applied` / `pending_review` / `stopped_for_review` / `failed`)を meta.yaml の `status` フィールドへ記録し(D111: turn status の永続化先)、次ターン番号の決定(最後に `applied` されたターン+1)と、未解決ターンによる後続ターンのブロックを実装する。`failed` ターンの再実行時は旧 artifact ディレクトリを `turn_NNNN_discarded_<n>` へ退避してから新規ディレクトリで実行する(上書き禁止、RNG 消費数の累積は退避分も合算。D112)。次ターン番号決定ロジックと discarded-dir 退避ロジックは、`add-session-autonomy` の GM review gate 事後操作(パイプライン外から state-model API と併用)から再利用できる公開ユーティリティとして提供する(D112)。
...
- `turn-pipeline`: 8 フェーズの実行順序、スロット Protocol(Simulate/Act/Resolve/BuildDiff/Check)、組み込み最小スロット、turn artifact 書き込み、meta.yaml、ターンステータス管理、失敗処理、commit-mode ランタイムパラメータを提供する。
...
- 自律進行(3ターン進行・シーン終了まで等)・停止条件判定・GM review gate の意思決定ロジック(`add-session-autonomy`)。commit-mode ランタイムパラメータはその暫定代替であり、恒久仕様ではない。
...
- `add-random-engine`(Resolve フェーズでの rng 消費数記録に必要。本 change の pass-through Resolve 実装自体は乱数判定を行わないが、`rolls.yaml` の空ファイル形式・meta.yaml の `rng_draws_consumed` フィールドは random-engine の契約に従う)。
...
- 依存する将来 change: `add-agent-runtime`(Simulate/Act/Resolve/BuildDiff/Check スロットの実装差し替え。BuildDiff は State Manager が本実装する)、`add-intervention`(Intervene フェーズの実装差し替え)、`add-session-autonomy`(commit-mode ランタイムパラメータを autonomy の判断ロジックへ置き換え、次ターン番号決定・discarded-dir 退避の公開ユーティリティを review gate 事後操作から利用)。
