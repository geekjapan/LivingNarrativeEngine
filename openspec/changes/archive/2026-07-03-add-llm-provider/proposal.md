...
- retry 2層の合成規則: 構造化出力 retry(バリデーション失敗、最大2回)と transport retry(接続/タイムアウト、最大2回)は独立にネストする — 構造化出力の各試行(初回+retry 2回の最大3回)ごとに transport retry(初回+retry 2回の最大3回)が適用され、1回の `complete()` あたりの HTTP リクエスト数は最大 3×3=9 回。呼び出しメタデータの所要時間・token 使用量は、成功・失敗を問わず実際に行われた全リクエストの合算を記録する。
