...
### D122(再掲・適用): LLM プロファイル resolver は静的な優先順位フォールバックのみ
- **決定**: resolver は binding key → プロファイル名の解決を `llm_bindings` の静的な辞書引きと固定の優先順位(`character:<id>` → `character_default` → 既定 `llm`、ロールは `<role>` → 既定 `llm`)のみで行う。失敗時の自動フォールバック(接続エラー時に別プロファイルへ切り替える等)は行わない。
- **理由**: ユーザー要件(キャラクターごと・進行役ごとに異なる LLM を明示的に割り当てたい)は静的な設定で満たせる。動的フォールバックは Non-Goals の「複数モデルのルーティング/フォールバック戦略」に該当し、エラー処理の複雑化を招くため YAGNI。
- **代替案**: 呼び出し失敗時に既定 `llm` へ自動フォールバックする案 → エラーの原因(接続エラーか、そのプロファイル固有の問題か)を呼び出し元が把握しづらくなり、失敗の透明性を損なう。不採用。
