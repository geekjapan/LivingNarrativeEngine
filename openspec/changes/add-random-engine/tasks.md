...
- [ ] 1.2 roll レコード生成のたびに project 内 roll id 通番(`roll_NNNN`)を1つ進める仕組みと、RNG draw 消費数(ロール種別ごとに固定: ダイス N 回 / 確率判定1回 / table 選択1回 / GM override 0回)を独立してカウントする仕組みを実装する
- [ ] 1.3 seed + RNG draw 消費数から RNG 状態を再構築する関数(空撃ちによる復元)を実装する
- [ ] 1.4 同一 seed からの2回初期化で同一列が得られることを検証するテストを書く
- [ ] 1.5 seed + 消費数からの状態再構築が継続 draw と一致することを検証するテストを書く
- [ ] 1.6 GM override 実行時に roll id は増加するが RNG 消費数は変化しないことを検証するテストを書く

## 2. ダイス記法パーサー

- [ ] 2.1 `NdM` `NdM+K` `NdM-K` をパースする関数を実装する(N≤100, M≤1000)
- [ ] 2.2 上限超過・不正記法に対する明確なパースエラーを実装する
- [ ] 2.3 パースしたダイスを RNG から draw して結果値(出目合計+修正値)を返す関数を実装する。省略可能な target 引数を受け取り、指定時は outcome(success/failure、結果値≥target)も返せるようにする
- [ ] 2.4 正常系・上限超過・不正記法のテストを書く(pytest, mock provider 不要のためユニットテストで十分)
- [ ] 2.5 target 指定時の outcome 算出(結果値≥target で success)のテストを書く
...
- [ ] 4.2 空エントリ・重み合計0・個々の重みが負のエントリに対するエラーハンドリングを実装する
...
- [ ] 6.1 reroll API(新規 roll id + `supersedes` 参照。元ロールと同じ種別・消費数で、現在の RNG ストリーム位置から新規 draw する)を実装する
- [ ] 6.2 GM override API(新規 roll id + `supersedes` 参照 + override フラグ。RNG は消費しない)を実装する
- [ ] 6.3 元レコードが変更されないこと、reroll が現在のストリーム位置から新規 draw すること、GM override が RNG を消費しないことを検証するテストを書く
