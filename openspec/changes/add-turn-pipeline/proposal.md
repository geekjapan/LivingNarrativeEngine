...
- `meta.yaml` に `status`・フェーズ別所要時間・LLM 呼び出し回数・`llm_tokens_total`(取得可能分の合計、取得不能時省略)・model・prompt hash・`rng_draws_consumed`(そのターンで消費した draw 数)・pipeline version を記録し、meta.yaml は完了マーカーとして最後に書く(D111)。
