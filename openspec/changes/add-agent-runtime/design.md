...
### D6: 停止条件評価用フィールドとの接続(D123)

spec-foundation D123 は session-autonomy の停止条件(`character_death`/`heavy_roll_failure`/`scene_end`)を機械的に評価するための具体的フィールド(`CharacterState.status`/`SceneState.status`/Roll の `severity`)を定める。本 change はこれらのフィールドの「発行側」を担う: Conflict Resolver は争われている行動の重大性が高いと判断した場合に roll 呼び出しへ `severity: critical` を明示指定できる(呼び出し側の裁量、random-engine 側の自動判定ではない)。State Manager はキャラクターの死亡・シーン終了を表す resolved event から `status: dead`/`status: ended` の diff 候補を生成する。これにより D123 が要求する評価材料が turn artifact(`rolls.yaml`/`state_diff.yaml`)上に確実に現れる。
