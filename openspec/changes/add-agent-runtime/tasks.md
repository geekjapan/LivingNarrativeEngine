...
- [ ] 1.2 `build_narrator_context` を実装する
- [ ] 1.3 `build_world_context`(World Simulator / State Manager 用の全状態参照コンテキスト、出力タグ付け前提)を実装する
- [ ] 1.4 直近 N 件イベント切り詰め + 関連 relationships 抽出ロジックを実装する
- [ ] 1.5 adversarial fixture(他者 private_mind・gm_vault・hidden_from イベントを含む WorldStateBundle)を用いた単体テストを作成し、spec の各 Scenario を再現する
- [ ] 1.6 narrator コンテキストの hidden_facts 除外・reader 可視イベントのみ抽出のテストを作成する

## 2. Agent I/O スキーマ

- [ ] 2.1 `CharacterAgentInput` / `CharacterAgentOutput`(action_candidates / emotion_deltas / goal_updates)を Pydantic v2 で定義する
- [ ] 2.2 `WorldSimulatorOutput`(parameter_drifts / faction_moves / background_events)を定義する
- [ ] 2.3 `ConflictResolverOutput`(resolved_events: state-model Event 準拠)を定義する
- [ ] 2.4 `StateManagerOutput`(state_diff: state-model StateDiff 準拠)を定義する
...
- [ ] 5.2 同一対象/排他的結果の衝突検出ロジックを実装する
- [ ] 5.3 design.md D4 の順序ポリシー(intervention 優先 → active_characters 順 → 背景イベント)を実装する
- [ ] 5.4 contested な結果に対する random-engine への roll 要求を実装する
- [ ] 5.5 resolved event(state-model Event 準拠、元候補の追跡可能性)の生成を実装する
- [ ] 5.6 衝突検出・順序・roll 連携のテストを作成する

## 6. State Manager

- [ ] 6.1 `living_narrative.agents.state_manager` に resolved events → state diff 候補生成ロジックを実装する
- [ ] 6.2 `source_event` を持たない変更候補の生成時拒否を実装する
- [ ] 6.3 現在状態に対する diff 変更の検証(存在しない id 参照等)を実装する
- [ ] 6.4 拒否ケース・正常ケースのテストを作成する

## 7. Turn Pipeline スロット統合

- [ ] 7.1 add-turn-pipeline の Simulate/Act/Resolve/Commit スロットを Context Builder/Character Agent/World Simulator/Conflict Resolver/State Manager の実装で置き換える
...
- [ ] 9.3 任意の LLM ベース漏洩評価(既定 warn)を実装する
...
- [ ] 10.2 任意の LLM ベース canon 矛盾検査(既定 warn)を実装する
