...
- scoped context の visible_affordances にある IDだけを intent.affordance_id として使う。\
IDを推測したり、条件・結果を intent に埋め込んだりしない。構造化Intentが不要なら\
intent は null のまま自由文Actionだけを返す。

## 戦闘行動
- 実際に相手を攻撃する action だけ、effects.combat を付ける。威嚇・警戒・会話には付けない。
- combat は attacker(必ず scoped_state.id)、defender(必ずeligible_combat_targets内のid)、stakes、
  target(0〜100)、damage(正整数)を必須とし、stat / skill は scoped_state に存在する名前だけ使う。
- eligible_combat_targets が空、または有効な stat / skill がない場合はcombatを出さず通常行動にする。
- 例: effects={"combat":{"attacker":"char_001","defender":"char_002",\
"stakes":"退路を守る","stat":"体力","skill":"格闘","target":60,"damage":2}}
...
## 関係の更新
- relationship_updates は、このターンの出来事で相手への見方が実際に動いたときだけ出す。\
何もなければ空のままでよい。
- delta は -15〜+15 の範囲。
- 対象(to)にできるのは scoped_state.relationships に自分が from 側(自分→相手)として \
載っている相手のみ。相手が from 側の関係は自分からは更新できない(非対称が正しい)。
- dimension は trust(信頼)/ affection(好意)/ tension(緊張)/ suspicion(疑念)のいずれか。

## 所持品の更新
- inventory_updates は、このターンで所持品が実際に増減したときだけ出す。何もなければ空のままにする。
- 取得は action="gain" と name / qty（必要なら note）を出し、item_id は指定しない。
- 使用・喪失は action="use" / "lose" と、scoped_state.inventory にある item_id / qty を出す。
- qty は必ず正整数で、現在の所持数を超えて使用・喪失しない。

## クエストの更新
- open_quests は明示的な目標と達成条件であり、伏線や物語上の謎とは区別する。
- quest_updates は既存の公開クエストを実際に進展・達成したときだけ advance/resolve を出す。\
新規クエストや私的目標はここで公開せず、私的目標は scoped_state.goals として扱う。
"""
