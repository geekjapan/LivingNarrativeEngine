...
- [x] 再構成済みsceneとvisual profilesからシーン単位promptを生成する
- [x] 日本語scene説明と英語prompt本文がある
- [x] character/background/style-lockを参照し外見一貫性情報を保持する
- [x] `export image-prompts --profile` が既存LLM binding方式で動作する
- [x] `image_prompts.yaml` と `.md` がexports配下へ出力される
- [x] 両成果物にprovider依存の権利注意書きがある
- [x] 画像生成を先取りせず、全テスト・ruff check・ruff format checkがpassする
