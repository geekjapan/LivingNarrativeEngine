...
- [ ] stat/skillから決定的なmodifierを導出し、難易度targetを含む判定を実行できる
- [ ] `dice_roll_request` から判定要求でき、既存の単純dice要求も壊さない
- [ ] 判定結果が既存形式の `rolls.yaml` に記録される
- [ ] 対応イベントに判定の `roll_ids` が設定される
- [ ] 欠落キャラクター・欠落能力・境界値・再現性のテストがある
