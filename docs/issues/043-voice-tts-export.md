...
- [x] voice profile schemaと`voice_profiles.yaml` optional load/save経路がある
- [x] VN scriptからdialogue/narrationの決定論的TTS台本を生成できる
- [x] dialogueはcharacter voice、地の文はnarrator/default voiceを参照する
- [x] `VoiceProvider` Protocol、dict registry、決定論的mockだけがある
- [x] CLIが`tts_script.yaml`と`.md`をatomicに出力する
- [x] reader可視台本だけをproviderへ渡し、未知speaker/profileを安全に扱う
- [x] mist_station実値と音声権利注意書きがある
- [x] 041とのCLI/media統合後も両subcommandが動き、全テスト・ruffがpassする
- [x] 無関係変更がなく、GitNexus `detect_changes`で影響範囲を確認している

## 完了記録

実装commitは`a49a485`。レビューでfactory registry統合、strict artifact schema、atomic一時ファイルcleanup、1-based turn制約を是正し、Wave 6最終統合状態で886 testsとruffの通過を確認した。
