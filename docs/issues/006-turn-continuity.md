...
- [ ] Commit相が適用済みターンのイベントを `timeline.yaml` に追記する(既存 `TimelineEntry` スキーマ、visibility保持)
- [ ] Act相の character context に直近Nターンの可視イベント(timeline由来、visibilityフィルタ済み)が入る
- [ ] emotion_deltas / goal_updates をState Manager経由でdiff化して適用する(clamp 0-100、visibility=character)
- [ ] mist_station実LLMで6ターン回し、ビート再演が解消しキャラ感情が推移することを確認
