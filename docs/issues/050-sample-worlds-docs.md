...
- [x] mist_stationと異なる日本語サンプル世界が1つ追加される
- [x] character 3〜4人、GM秘密、hidden facts、factions、threats、quest、visual profilesを含む
- [x] registry登録済みで`init --template`から選択できる
- [x] モデル・参照整合・leak checkerを含むテンプレート検証テストがある
- [x] `docs/rights-and-security.md`に画像権利、ローカル運用、秘密情報の注意がある
- [x] READMEから注意書きと新テンプレートへ到達できる
- [x] 034取込み後にquest fixtureの正規loadを検証している
- [x] 全テスト、ruff check、ruff format checkがpassする
- [x] 無関係変更がなく、GitNexus `detect_changes`で影響範囲を確認している

## 検証記録

- 2026-07-11: 034取込み後のworktreeで`NO_COLOR=1 uv run pytest` 825件、
  `uv run ruff check .`、`uv run ruff format --check .`がpass。
- 2026-07-11: 公開clueとGM真相を分離し、公開面の非漏洩と秘密本文の検出、
  Quest正規load、GitNexus影響範囲を二軸レビューで確認。
