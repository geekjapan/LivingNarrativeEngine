...
- [ ] combat payloadを境界で検証し、attacker/defender/stakesを保持する
- [ ] 既存Resolveスロット内だけでstats/skills判定を再利用する
- [ ] 1交戦のrollが`rolls.yaml`とevent `roll_ids`へ記録される
- [ ] damageが`stats.hp`へのclamp付き`StateDiff(op=delta)`になる
- [ ] `life_or_death`重大失敗がGM review用stop conditionへ連動し、即死させない
- [ ] 非combat Resolve、決定性、未知character/能力値、不正damageを回帰テストする
- [ ] 新pipeline phaseと直接state mutationがない
