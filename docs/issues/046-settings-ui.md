...
- [x] 設定ページでLLM profiles/bindingsとpricingを閲覧できる
- [x] 有効な設定を編集・保存し、再読込できる
- [x] 不正YAML・不正profile/binding・不正価格型を明確に拒否し、既存ファイルを変更しない
- [x] 設定書込みAPIのpath traversalを拒否する回帰テストがある
- [x] 書込みがatomicで、localhost bind固定の既存保証を壊さない
