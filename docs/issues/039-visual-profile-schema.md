...
- [ ] character/background/style-lockのPydanticモデルが明示的な正フィールドで定義される
- [ ] visual profileファイルのoptional loadがあり、欠落時も既存projectを読める
- [ ] 新規モデルのJSON Schemaがexportされる
- [ ] mist_stationテンプレートにschema-validな実値1式がある
- [ ] web GM characters paneにescapedなvisual profile要約が1行表示される
- [ ] 画像生成などのruntime消費を先取りせず、全テスト・ruff check・ruff format checkがpassする
