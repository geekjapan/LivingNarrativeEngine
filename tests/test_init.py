...
def test_minimal_factions_yaml_is_generated_with_an_empty_list(tmp_path):
    # project-workspace/spec.md (add-project-foundation): factions.yaml is part of the
    # workspace layout unconditionally. Templates may override it with their own data.
    output = tmp_path / "proj"

    create_project(output, title="x", template="minimal")
...
def test_mist_station_template_generates_exactly_one_faction(tmp_path):
    output = tmp_path / "mist_station"
    ...
    assert bundle.factions[0].id == "faction_001"
