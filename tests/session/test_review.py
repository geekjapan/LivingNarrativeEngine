...
from living_narrative.session.review import (
    ReviewDecision,
    ReviewStateError,
    resolve_review,
    write_artifact_diff,
...
def test_resolving_applied_turn_raises_without_touching_state_or_history(tmp_path, build_project):
    project_path = build_project(tmp_path)
    workspace = project_path.parent / "workspace"
    turn_dir = workspace / "runs" / "turn_0001"
    _pending_turn(
        project_path,
        turn_dir,
        StateDiff(id="diff_0001", turn=1, changes=[_change("should not apply")]),
    )
    (turn_dir / "meta.yaml").write_text(
        yaml.safe_dump({"turn": 1, "status": "applied"}),
        encoding="utf-8",
    )
    before_summary = StateStore.load(workspace / "state").world.summary

    with pytest.raises(ReviewStateError, match="status=applied"):
        resolve_review(
            workspace_root=workspace,
            state_dir=workspace / "state",
            turn_dir=turn_dir,
    ...
    assert StateStore.load(workspace / "state").world.summary == before_summary
    assert not (workspace / "interventions.yaml").exists()


def test_resolving_failed_turn_raises(tmp_path, build_project):
    project_path = build_project(tmp_path)
    workspace = project_path.parent / "workspace"
    turn_dir = workspace / "runs" / "turn_0001"
    _pending_turn(project_path, turn_dir, StateDiff(id="diff_0001", turn=1))
    (turn_dir / "meta.yaml").write_text(
        yaml.safe_dump({"turn": 1, "status": "failed"}),
        encoding="utf-8",
    )

    with pytest.raises(ReviewStateError, match="status=failed"):
        resolve_review(
            workspace_root=workspace,
            state_dir=workspace / "state",
            turn_dir=turn_dir,
